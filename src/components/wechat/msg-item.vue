    ...
    <li :class="{'item-hide':deleteMsg}">
        ...
        <router-link :to="{ path: '/wechat/dialogue', query: { mid: item.mid,name:item.group_name||(item.user[0].remark||item.user[0].nickname),group_num:item.user.length}}" tag="div" class="list-info" v-swiper v-on:click.native="toggleMsgRead($event,'enter')">
            ...
            <div class="desc-box">
                ...
                <!-- 接入 -->
                <div v-show="this.$store.state.user.status == 2" class="access">接入</div>
                <div class="desc-msg">
                    <span v-show="item.type=='group'">{{item.msg[item.msg.length-1].name}}:</span>
                    <span>{{item.msg[item.msg.length-1].text}}</span>
                    <span v-show="this.$store.state.user.status == 1 || this.$store.state.user.status == 4" style="float:right">9.20</span>
                    <span v-show="this.$store.state.user.status == 2 || this.$store.state.user.status == 3" style="float:right" class="waitting">等待时间：  {{item.msg[item.msg.length-1].date | fmtDate('yy-mm-dd hh:ss')}}</span>
                </div>
                <!-- <div class="desc-msg" style="width: 100%; text-align: right">
                  {{item.msg[item.msg.length-1].date | fmtDate('yy-mm-dd hh:ss')}}
                </div> -->
            </div>
        </router-link>
